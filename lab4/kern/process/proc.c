...
alloc_proc(void) {
     ...
     *procstruct中的以下字段需要初始化

    *enum proc_state状态；//进程状态

    *int pid；//进程ID

    *int runs;//Proces的运行时间

    *uintptr_t kstack；//进程内核堆栈

    *volatile bool need_sched；//bool值：是否需要重新安排以释放CPU？
    ...
    *uintpttr_t cr3；//CR3寄存器：Page Directroy表（PDT）的基地址
