...
alloc_proc(void) {
     ...
     */
     ...
     //LAB5 2112189 : (update LAB4 steps)
     /*
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    */
    ...
    //1.调用alloc_proc()函数申请内存块
    if ((proc = alloc_proc()) == NULL) {
        goto fork_out;
    }
    //将子进程的父节点设置为当前进程
    proc->parent = current;
    // update step 1:确保当前进程的wait_state是0
